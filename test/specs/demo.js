...
describe('Rahul Shetty login page', () => {
    ...
    it('Chai assertions for UI controls',()=>{
        ...
        const radioOptions = $$(".customradio") 
        const options = radioOptions[1] //user radiobutton which gives pop up
        ...
        //Check whether Admin option is selected or not -It will return false since user is selected
        $$(".customradio")[0].$("span").isSelected()
 
         //Press okay button in popup - popup comes for user option only so selected readio[1]
         //since selected okay button it will move to Admin option and 
         //returns false now as user is not selected
         options.$("span").click()  
         modal.waitForDisplayed()
         $("#okayBtn").click()
         console.log("Selected value result :" + options.$("span").isSelected())
         
         //validate popup is not shown when admin is selected
         expect(modal).not.toBeDisplayed()
 
         const ddown = $("select.form-control")
